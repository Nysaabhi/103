    ...
    display: none !important; 
      ...
      .deals-button i { 
        ...
        <body>
            ...
            <script>                        
                // Configuration object for search, chat, and math functionality
const searchConfig = {
    qaDatabase: {
...
class EnhancedMathSolver {
    ...
    detectMathQuery(query) {
        ...
        // Check for direct math keywords
        const hasMathFunction = Object.keys(this.mathFunctions)
            .some(func => normalizedQuery.includes(func));
        
        // Check for geometry keywords
        const hasGeometryKeyword = Object.keys(this.geometryKeywords)
            .some(type => normalizedQuery.includes(type));
        
        // Check for operator keywords
        const hasOperatorKeyword = Object.keys(this.basicOperators)
            .some(op => normalizedQuery.includes(op));
        
        // Check for numbers (including decimals)
        const hasNumbers = /\d+(\.\d+)?/.test(normalizedQuery);
        
        // Check for advanced math patterns
...
// Update the ChatSystem class to use the new EnhancedMathSolver
class ChatSystem {
    constructor() {
        this.messageHistory = [];
    ...
    processQuery(query) {
        ...
        if (this.mathSolver.detectMathQuery(query)) {
            const mathSolution = this.mathSolver.solveMathProblem(query);
            if (mathSolution) {
                return mathSolution;
            }
        }

        // If not a math query or math solving failed, proceed with existing logic
...
// Add necessary styles for math-related messages
